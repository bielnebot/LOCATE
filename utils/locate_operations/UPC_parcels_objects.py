...
def get_ibi_fieldset() -> FieldSet:
    """
    Carga los ficheros de IBI
    """
    base_path = Path(cfg.data_base_path)
    # dimensions = {'time': 'time', 'depth': 'depth', 'lon': 'longitude', 'lat': 'latitude'}
    dimensions = {'lat': 'latitude', 'lon': 'longitude', 'time': 'time'}

    # Cargamos los ficheros currents IBI
    dir_ibi = base_path / Path(cfg.IBI_files_dir)
    assert dir_ibi.is_dir(), f"Directory not found: {dir_ibi}"
    basepath_ibi = '*.nc'
    filenames_ibi = sorted(dir_ibi.glob(str(basepath_ibi)))  # la llista de fitxers que hi ha a la carpeta

    U_ibi = Field.from_netcdf(filenames_ibi, ('U', 'uo'), dimensions, fieldtype='U', allow_time_extrapolation=True)
    V_ibi = Field.from_netcdf(filenames_ibi, ('V', 'vo'), dimensions, fieldtype='V', allow_time_extrapolation=True)
    print("type(U_ibi) = ", type(U_ibi))
    print(U_ibi)
...
#
#     # Cargamos los ficheros del dominio Costero
#     dir_ibi = base_path / Path(cfg.IBI_files_dir + '_resampled/')
